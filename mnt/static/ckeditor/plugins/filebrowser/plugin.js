    ...
    function attachFileBrowser( editor, dialogName, definition, elements )
    {
        ...
        for ( var i in elements )
        {
            ...
            if ( typeof element.filebrowser == 'string' )
            {
                var fb =
                {
ion : ( element.type == 'fileButton' ) ? 'QuickUpload' : 'Browse',
get : element.filebrowser
                };
...
( url === undefined )
 url = editor.config.filebrowserBrowseUrl;
                ...
                if ( url )
                {
ment.onClick = browseServer;
ment.filebrowser.url = url;
ment.hidden = false;
                }
            }
...
ment.filebrowser.url = url;
ment.hidden = false;
upFileElement( editor, definition.getContents( element[ 'for' ][ 0 ] ).get( element[ 'for' ][ 1 ] ), element.filebrowser );
                }
            }
        }
    }
    ...
    CKEDITOR.on( 'dialogDefinition', function( evt )
    {
        ...
        for ( var i in definition.contents )
        {
            if ( ( element = definition.contents[ i ] ) )
            {
                ...
                {
ment.hidden = !isConfigured( definition, element[ 'id' ], element.filebrowser );
                }
            }
        }
    } );
